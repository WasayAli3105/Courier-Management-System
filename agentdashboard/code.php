...
if(isset($_POST['courierUpdate'])) {
    ...
    if($update_query) {
        ...
        if ($row = mysqli_fetch_assoc($check_query)) {
            // $tracking_id = strtoupper(substr(md5($id), 0, 10)); // Generate a unique tracking ID
            $tracking_id = $row['tracking_no'];
            $senderEmail = $row['sender_email'];
            $receiverEmail = $row['receiver_email'];
            $parcelWeight = $row['parcel_weight'];
            // $deliveryDate = date('Y-m-d', strtotime('+3 days')); // Estimated delivery in 3 days
            $deliveryDays = ($row['courier_type'] == 'fast') ? 1 : 3;
            ...
            try {
                ...
                // Email Recipients
                $mail->setFrom('no-reply@yourcourier.com', 'SwiftShipCourier Service');
                ...
                <body style='font-family:Arial, sans-serif; line-height:1.6;'>
                    ...
                    <table style='border-collapse:collapse; width:100%; border:1px solid #ddd;'>
                        ...
                        <tr><td><b>Parcel Price:</b></td><td>$price Rs</td></tr>
                    ...
                    <p>Thank you for using <b>SwiftShip Courier Service</b>. If you have any questions, contact our support team.</p>
                    <hr>
                ...
                if ($mail->send()) {
                    echo "<script>alert('Email has been sent successfully!'); window.location.href='public.php?dashboard&status=success';</script>";
                    exit();
                } else {
                    echo "<script>alert('Email could not be sent. Error: " . addslashes($mail->ErrorInfo) . "'); window.location.href='public.php?dashboard&status=failed';</script>";
                    exit();
                }
...
if (isset($_POST['senderName'])) {
    ...
    if ($stmt->execute()) {
        ...
        location.assign('public.php?courier-add');
        </script>";
...
//     // for ($i = 0; $i < 8; $i++) { // 8-character random string
//     //     $randomCode .= $characters[rand(0, strlen($characters) - 1)];
//     // }

//     // $dateStamp = date('ymd'); // Format: YYMMDD

//     // $trackingNumber = "TRK-" . $dateStamp . "-" . $randomCode;
...
//     // $price_query = mysqli_query($connection,"select parcel_weight, (parcel_weight * 1500) AS price from courier");
...
if(isset($_POST['courierUpdate'])){
    ...
    $update_query = mysqli_query($connection,"Update courier set receiver_address = '$r_address',price='$price', parcel_weight='$parcel',status='$status' where courier_id = '$id'");

    if($update_query){
        echo "<script>
        alert('Courier has been Updated');
        location.assign('/admindashboard/public.php?courier-details')
        </script>";
    }
}
...
//     $nameRe = /^[a-z\s]{3,}$/;
//     $email = $("email").val();
//     $emailRE = /^[\w]{2,}[@][a-z]{5,9}[.][a-z]{3}$/;
//     $password = $("#password").val();
//     $passwordRE = /^(?=.*[A-Z])(?=.*[a-z])(?=.*[\d])(?=.*[!@$%&*])[A-Za-z\d!@$%&*]{8,15}$/;
//     if (!($name && $email && $password )) {
//         EmtInput("$name");
//         EmtInput("$email");
//         EmtInput("$password");

//     } else if ((!$nameRe.test($name)) || (!$emailRE.test($email)) || (!$passwordRE.test($password)) {
